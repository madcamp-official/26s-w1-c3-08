...
3. DB schema는 Backend 담당자가 관리한다.
4. Figma와 화면 품질은 Frontend 담당자가 관리한다.
5. packages/shared 변경은 둘 다 리뷰한다.
6. env 추가는 .env.example과 README에 반영한다.
7. 배포 전 각자 담당 영역의 build/typecheck를 통과시킨다.
8. 운영 secret은 repository에 넣지 않는다.
9. 서로의 영역을 수정해야 하면 PR 설명에 이유를 적는다.
10. 화면과 API가 동시에 바뀌는 기능은 contract를 먼저 합의한다.
```
