...
export default function ArrivalPage() {
  ...
  return (
    <main className="mx-auto min-h-screen max-w-3xl px-4 py-10">
      ...
      {message && opened ? (
        <article className={`rounded-md border border-slate-200 bg-white p-6 shadow-soft ${themeClass(message.theme)}`}>
          ...
          {message.canReply ? (
            <div className="mt-6 rounded-md border border-slate-200 bg-white p-4">
              <p className="text-sm font-semibold text-ink">익명 답장</p>
              <textarea
                value={replyContent}
                ...
                className="focus-ring mt-3 w-full resize-y rounded-md border border-slate-300 px-3 py-2 text-sm"
              />
              <div className="mt-3 flex flex-wrap items-center gap-2">
                ...
                <p className="text-xs text-slate-500">답장 내용도 안전 검사를 통과한 뒤 저장돼요.</p>
              </div>
          ...
          <button
            ...
            className="focus-ring mt-4 inline-flex items-center gap-2 rounded-md border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-700"
          >
            <ShieldAlert size={16} />
            신고
          </button>
