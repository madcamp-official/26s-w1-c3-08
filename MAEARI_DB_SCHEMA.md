...
```txt
Message
  -> 편지 본문, 예약일, 발신자, 숨김 옵션, AI 검사 상태, 도착 옵션

MessageRecipient
  -> 수신자별 정보, 귀속 사용자, 열람 상태, 발송 상태

MessageAttachment
  -> 메시지별 첨부 이미지 metadata
...
이 구조를 사용하면 단일 수신자와 그룹 전송 모두 같은 `Message`에 하나 이상의 `MessageRecipient`를 붙이는 방식으로 처리할 수 있습니다.
...
`friendCode`와 `nickname`은 친구 찾기 화면에서 검색 후보를 만들 때 사용합니다. 검색 결과에서는 이미 친구이거나 대기 중인 친구 요청이 있는 사용자, 삭제/정지된 사용자를 제외합니다.
...
- `theme`: 공개 링크와 상세 화면에 적용할 봉투/편지 테마
- `isReplyEnabled`: 공개 링크에서 익명 답장 허용 여부
- `sentAt`: 실제 발송 처리 시각
...
메시지 하나에는 한 명 이상의 수신자를 만들 수 있습니다. 단일 전송은 `MessageRecipient` 1개, 그룹 전송은 여러 개를 생성합니다.
...
- `receiverArchivedAt`: 수신자가 받은 마음을 아카이브한 시각
...
첨부 이미지의 metadata를 저장합니다. 파일 자체는 `UPLOAD_DIR` 아래에 저장하고, API가 `/api/uploads/*`로 정적 제공하는 public URL을 DB에 저장합니다.

주요 필드:

- `messageId`: 첨부가 속한 메시지
- `publicUrl`: 브라우저가 접근할 수 있는 이미지 URL
- `storageKey`: 업로드 디렉터리 기준 상대 경로
- `originalName`: 원본 파일명
- `mimeType`: 허용 이미지 MIME type
- `sizeBytes`: 저장된 파일 크기

## 3.5 MessageReply

공개 링크를 열람한 수신자의 익명 답장을 저장합니다. 답장도 저장 전 AI moderation을 통과해야 하며, 관리자는 필요 시 답장을 숨김 처리할 수 있습니다.

주요 필드:

- `messageId`: 답장이 달린 메시지
- `messageRecipientId`: 답장을 남긴 공개 링크 수신자
- `content`: 답장 본문
- `senderDisplayName`: 익명이 아닌 답장을 허용할 때의 표시명
- `isAnonymous`: 익명 여부
- `status`: `VISIBLE`, `HIDDEN`, `DELETED`
- `moderationInputHash`, `moderationCategories`: 답장 검사 이력 요약
- `hiddenAt`, `hiddenReason`: 관리자 숨김 처리 정보

## 3.6 MessageAccessToken
...
메시지 신고와 관리자 검토 상태를 저장합니다. 공개 링크 수신자는 token 기반으로 신고하고, 로그인 사용자는 본인이 접근 가능한 메시지를 신고할 수 있습니다.

주요 필드:

- `messageId`: 신고 대상 메시지
- `messageRecipientId`: 공개 링크 또는 수신자 맥락
- `reporterUserId`: 로그인 신고자. 비회원 공개 링크 신고는 null
- `reason`, `details`: 신고 사유와 상세 설명
- `status`: `PENDING`, `REVIEWED`, `DISMISSED`
- `reviewedAt`, `reviewNote`: 관리자 검토 결과
...
5. 단일 또는 그룹 수신자 수만큼 MessageRecipient 생성
6. 첨부 이미지가 있으면 MessageAttachment 생성
7. 수신자별 MessageAccessToken 생성
8. publicUrl/publicUrls 반환
```
...
6. 첨부 이미지가 있으면 MessageAttachment 생성
7. MessageAccessToken은 생성하지 않음
8. publicUrl = null 반환
9. moderationNextRetryAt = now + 1 day
```
...
```txt
1. ArrivalHintScheduler가 Message 조회
2. 조건: Message.status = PENDING
3. 조건: Message.hintAt <= now
4. 조건: Message.hintSentAt IS NULL
5. NotificationProcessor가 eventType = ARRIVAL_HINT로 수신자별 알림 처리
6. NotificationLog 생성 또는 재사용
7. Message.hintSentAt = now 기록
```
...
아카이브
1. 수신자가 받은 마음에서 보관 버튼 클릭
2. PATCH /api/messages/:id/archive
3. 현재 사용자의 MessageRecipient.receiverArchivedAt = now
4. /inbox에서는 제외되고 /archive에서 조회
5. PATCH /api/messages/:id/unarchive로 receiverArchivedAt = null 복구

오래된 메시지 정리 정책
1. v1은 자동 hard delete를 수행하지 않음
2. 사용자가 직접 삭제하면 senderDeletedAt 또는 receiverDeletedAt만 기록
3. Message, MessageAccessToken, ModerationLog, NotificationLog는 감사 추적을 위해 보존
```
...
2. message.status = SENT, isReplyEnabled = true 확인
3. 답장 본문 moderation
4. MessageReply 생성
5. 발신자는 /messages/[id]에서 답장 확인

신고
1. /arrival/[token] 또는 /messages/[id]에서 신고
2. MessageReport 생성
3. 관리자는 /admin에서 REVIEWED 또는 DISMISSED 처리
4. 필요하면 User.suspendedAt, suspensionReason 기록
```

## 5.10 관리자 발송 통계

```txt
1. /api/admin/overview가 NotificationLog를 상태별, 채널별, provider별로 groupBy
2. NotificationLog.errorCode를 집계해 실패 원인 표시
3. NotificationLog.status = PENDING AND nextRetryAt <= now는 즉시 재시도 대상으로 표시
4. MessageRecipient.deliveryStatus를 집계해 수신자별 최종 발송 상태 표시
```
...
이 스키마는 현재 구현된 확장 기능을 같은 구조 안에서 처리하고, 이후 provider나 정책을 교체해도 큰 구조 변경 없이 고도화할 수 있도록 설계되어 있습니다.

- 현재 구현: 그룹 전송은 `MessageRecipient` 여러 개 생성으로 처리
- 현재 구현: 도착 전 힌트는 `NotificationEventType.ARRIVAL_HINT`와 `hintAt`, `hintSentAt`으로 처리
- 현재 구현: 익명 답장은 `MessageReply` 모델로 처리
- 현재 구현: 이미지 첨부는 `MessageAttachment` 모델로 처리
- 현재 구현: 감정 리포트는 `Message.emotionTag`, `MessageRecipient.readAt`, `Message.status` 기반 집계
- 현재 구현: 이메일/SMS 수신거부는 `ContactSuppression.channel + contactHash` unique key로 처리
- 향후 확장: 카카오 알림톡은 `NotificationLog.channel = KAKAO_ALIMTALK`에 provider만 추가
- 향후 확장: SMS provider는 Solapi 외 provider dispatcher 추가
- 향후 확장: 이메일 provider는 Gmail SMTP에서 SES/SendGrid/Resend 등으로 교체
- 향후 확장: 수신거부 관리자 해제, abuse 대응, 연락처 기반 차단 정책 추가
- 향후 확장: 친구 차단, 친구 초대 링크, 연락처 기반 추천 추가
