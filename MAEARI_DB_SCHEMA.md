...
- `PUBLIC_TOKEN_ENCRYPTION_KEY`는 마음나무 share token 복호화에 사용합니다. 비어 있으면 `PUBLIC_TOKEN_PEPPER` 기반 키를 사용하므로 운영 secret 변경 시 기존 마음나무 URL 복원이 깨질 수 있습니다.
...
Message
  -> 편지 본문, 예약/도착 상태, 발신자 표시명 snapshot, 서버가 선택한 인증 PHONE snapshot, 숨김 옵션, AI 검사 상태
...
NotificationLog
  -> IN_APP/Gmail SMTP/Solapi 발송 이력, 사용자 읽음 상태, 답장/마음나무 알림, retry, provider id
...
DailyLine / DailyLineSelection
  -> KST 날짜별 OpenAI 생성 오늘의 한 줄과 선택 이력
```
...
- `signupPhoneVerificationRequiredAt`: 신규 카카오 가입 직후 PHONE 인증을 강제해야 하는 시각. 기존 사용자는 `NULL`
- `signupPhoneVerificationCompletedAt`: 가입 직후 PHONE 인증 완료 시각
...
- `emotionTag`, `customEmotionTag`: 감정 태그. `EmotionTag.CUSTOM`은 직접 입력 태그이자 목록/리포트의 “기타” 그룹이며, API는 목록/리포트에서 `customEmotionTag=null`로 묶고 상세/public에서만 정제된 직접 입력 문구를 반환합니다.
...
공개 링크를 열람한 수신자의 익명 답장과 로그인 수신자가 작성한 답장을 저장합니다.

주요 필드:

- `messageId`
- `messageRecipientId`
- `authorUserId`: 로그인 수신자가 작성한 답장일 때 작성자 user id. 공개 링크 답장은 `null`
- `content`
- `senderDisplayName`
- `isAnonymous`
...
- `authorDeletedAt`: 작성자가 자신이 쓴 답장 목록에서 숨긴 시각
...
- `/api/messages/replies/sent`는 `authorUserId`, `authorDeletedAt`을 기준으로 내가 작성한 답장을 조회합니다.
...
- `readAt`: 사용자 인앱 알림 읽음 시각. unread badge는 `targetUserId + IN_APP + SENT + readAt IS NULL` 기준
...
- 기존 받은 마음/답장 읽음 처리와 사용자 알림 읽음 처리는 함께 갱신해 badge와 상세 화면 상태가 어긋나지 않게 합니다.
...
## 3.11 DailyLine / DailyLineSelection

`오늘의 한 줄`을 매일 안정적으로 노출하기 위한 OpenAI 생성 결과와 선택 이력을 저장합니다.

`DailyLine` 주요 필드:

- `text`: 노출할 글귀
- `poemTitle`: 시제목 또는 내부 문구 제목
- `poet`: 시인 또는 작성자
- `sourceNote`: OpenAI 모델과 KST 생성 날짜 등 운영 추적 메모
- `isActive`, `sortOrder`: rotation 대상과 순서

`DailyLineSelection` 주요 필드:

- `date`: KST 기준 `YYYY-MM-DD`, primary key
- `dailyLineId`: 해당 날짜에 선택된 `DailyLine`
- `selectedAt`: selection 생성 시각

운영 정책:

- mock/dummy/seed 문구를 만들지 않고 OpenAI API의 실제 응답만 저장합니다.
- scheduler는 `DAILY_LINE_CRON`으로 오늘 selection을 미리 만들고, `/api/daily-line`은 selection이 없으면 즉시 OpenAI 생성 후 저장합니다.
- OpenAI 실패 시 `DAILY_LINE_GENERATION_FAILED`로 실패합니다.
- 프롬프트는 꽃 관련 시 일부를 `글귀`, `시제목`, `시인` 순으로 반환하게 하며, 공개저작물로 판단 가능한 고전 시만 사용하도록 제한합니다.
- 같은 날짜에는 같은 문구를 유지합니다.

## 3.12 ContactSuppression
...
- `shareTokenEncrypted`: owner가 도착 전 같은 URL/QR을 다시 확인할 수 있도록 raw token을 AES-GCM으로 암호화한 값
...
- raw token은 평문 저장하지 않습니다. 생성 시 `shareTokenEncrypted`에 암호화 저장하고, `POST /api/message-collections/:id/share-link`에서 도착 전 같은 `collectionUrl`로 복원합니다.
- migration 이전 row처럼 `shareTokenEncrypted`가 없으면 최초 share-link 조회 시 한 번만 새 stable token을 생성해 저장합니다.
- `scheduledAt`이 지나면 `deliverDueMessageCollections` job이 `DELIVERED`로 전환합니다.
- owner가 `PATCH /api/message-collections/:id/close`를 호출하면 예정 시각 전에도 즉시 `DELIVERED`로 전환합니다.
- public `/tree/[token]` 링크는 `ACTIVE && scheduledAt > now`일 때만 조회/제출 가능하며, 도착 이후에는 `COLLECTION_LINK_EXPIRED`입니다.
- `DELETE /api/message-collections/:id/permanent`는 `MessageCollection` hard delete를 수행하고 submissions/collection notifications는 FK cascade로 삭제됩니다.
...
13. 신규 가입 직후 인증 대상이면 User.signupPhoneVerificationCompletedAt = now
```
...
```txt
1. 신규 가입자는 GET /api/me의 accountSetup.requiresSignupPhoneVerification으로 /phone-verification?next=/onboarding 먼저 유도
2. 기존 미인증 사용자는 /write가 GET /api/me/contacts로 writerEligibility.hasVerifiedStrictPhone 확인
3. 인증 PHONE이 없으면 /phone-verification?next=/write로 유도
4. POST /api/messages 요청
5. 서버가 senderContactId payload를 무시하고 active verified strict PHONE 직접 조회
6. 첨부 이미지가 있으면 MIME/확장자/용량/매직바이트 검사
7. 첨부 이미지가 있으면 Tesseract OCR로 이미지 속 텍스트 추출
8. 제목/본문/감정 태그/첨부 OCR 텍스트를 합쳐 OpenAI moderation + guardrail 검사
9. 통과
10. Message.status = PENDING 생성
11. Message.senderContactId, senderContactSnapshot 저장
...
8. /sent 답장함은 GET /api/messages/replies/received를 기본으로 사용하고, GET /api/messages/sent/replies는 frontend 이전 기간 alias로만 유지
```
...
2. raw token은 /tree/[token] URL에 포함하고 DB에는 tokenHash와 shareTokenEncrypted 저장
3. 비회원이 /tree/[token]에서 텍스트 편지 제출
4. IP hash rate limit 확인
5. 제출 content moderation 통과 시 MessageCollectionSubmission.VISIBLE 저장
6. scheduler가 ACTIVE AND scheduledAt <= now collection 조회 또는 owner가 /close 호출
...
11. public token URL은 도착 이후 만료되어 더 이상 조회/제출 불가
```
...
- `MessageCollection`
  ...
  - `tokenHash`와 AES-GCM 암호화 token을 저장하며 raw token 평문은 저장하지 않습니다.
  - `scheduledAt`이 지나면 scheduler가 `DELIVERED`로 전환하고, owner의 `/close` 호출도 같은 delivery helper를 사용합니다.
- `MessageCollectionSubmission`
...
- AppShell
  ...
  - navigation과 sidebar 이미지는 presentation layer입니다.
...
- QR 공유
  - `QrShare`는 공개 도착 URL 또는 마음나무 `collectionUrl`/share-link 응답 URL을 클라이언트에서 QR로 렌더링합니다.
...
- QR은 공개 URL을 시각화할 뿐 DB에 별도 QR 값을 저장하지 않습니다. 마음나무 QR은 생성 응답 또는 share-link 응답의 `collectionUrl`을 사용합니다.
...
`20260707120000_notifications_replies_daily_line`까지의 migration으로 다음 schema 변경이 코드 기준 최신 상태입니다.
...
| 발신자 표시명 | `Message.senderDisplayName` | 마음 작성 시점의 발신자 표시 이름 snapshot |
| 답장 작성자 | `MessageReply.authorUserId`, `authorDeletedAt` | 내가 쓴 답장과 내가 받은 답장 분리 |
| 사용자 알림 읽음 | `NotificationLog.readAt` | AppShell 종 아이콘 unread count와 알림 목록 |
| 오늘의 한 줄 | `DailyLine`, `DailyLineSelection` | KST 날짜별 문구 rotation |
...
| 마음나무 | collection token hash, encrypted share token, 제출 본문, 제출 IP hash | collection raw token 평문, raw IP |
...
| 감정 pill | 목록/리포트는 `Message.emotionTag`와 “기타” 그룹화된 `customEmotionTag=null`, 상세/public은 정제된 `customEmotionTag` |
