...
| `CommunicationBlock` | 마이페이지 송수신 거부 설정. 회원/이메일/전화번호 대상, `SEND_TO`/`RECEIVE_FROM` 방향별 차단 |
...
| GET | `/api/me` | 현재 로그인 사용자와 가입 설정 상태 조회 | 세션 쿠키 | `{ user, accountSetup: { hasVerifiedStrictPhone, requiresSignupPhoneVerification, hasCompletedOnboarding } }` |
| PATCH | `/api/me/onboarding` | 온보딩 답변 저장 | `{ note }` | `{ user }` |
| GET | `/api/time` | 서버 기준 현재 시각과 기본 예약 시각 조회 | 없음 | `{ serverNow, defaultScheduledAt }` |
| GET | `/api/daily-line` | KST 기준 오늘의 한 줄 조회. selection이 없으면 OpenAI로 생성 | 없음 | `{ dailyLine: { date, text, poemTitle, poet } }` |
...
| GET | `/api/me/communication-blocks` | 마이페이지 송수신 거부 목록 조회 | 세션 쿠키, optional `direction=SEND_TO\|RECEIVE_FROM` | `{ blocks }` |
| POST | `/api/me/communication-blocks` | 회원/이메일/전화번호 대상 송수신 거부 등록 | `{ direction, target }` | `{ block, created }` |
| DELETE | `/api/me/communication-blocks/:id` | 송수신 거부 설정 해제 | 세션 쿠키, block id | `{ deleted: true }` |
...
| GET | `/api/notifications/summary` | 로그인 사용자의 읽지 않은 인앱 알림 수 | 세션 쿠키 | `{ unreadCount }` |
| GET | `/api/notifications` | 로그인 사용자의 인앱 알림 목록 | 세션 쿠키, `limit` query | `{ unreadCount, notifications }` |
| PATCH | `/api/notifications/:id/read` | 인앱 알림 단건 읽음 처리 | 세션 쿠키, notification id | `{ read, readAt }` |
| PATCH | `/api/notifications/read-all` | 인앱 알림 전체 읽음 처리 | 세션 쿠키 | `{ read, readAt, updatedCount }` |
...
| GET | `/api/public/message-collections/:token` | 도착 전 공개 마음나무 조회. 도착 이후 `410 COLLECTION_LINK_EXPIRED` | 공개 token | `{ collection }` |
| POST | `/api/public/message-collections/:token/submissions` | 도착 전 비회원 마음나무 편지 제출. 도착 이후 `410 COLLECTION_LINK_EXPIRED` | 공개 token, `{ senderDisplayName?, content }` | `{ submission }` |
...
1. `detectKoreanAbuse`: 한국어 욕설, 비하어, 기호/숫자/공백 삽입, 자모 축약, 의도적 오타 obfuscation을 로컬 규칙으로 즉시 차단합니다.
...
guardrail prompt는 `apps/api/src/modules/moderation/moderation-policy.ts`에 있습니다. 이 prompt는 매아리가 “수신자에게 직접 전달되는 예약 메시지 서비스”임을 설명하고, 욕설/혐오/비하/성적 모욕/사전식 욕설 목록과 Korean filter bypass 형태도 수신자에게 전달되면 차단하도록 지시합니다.
...
- `apps/web/public/images/maeari-message-default-1.png` ~ `maeari-message-default-4.png`: 첨부 이미지가 없는 마음 목록 card의 기본 thumbnail 후보
- `apps/web/public/images/maeari-envelope-theme-lavender.png`, `maeari-envelope-theme-moss.png`, `maeari-envelope-theme-sunset.png`, `maeari-envelope-theme-midnight.png`, `maeari-envelope-theme-paper.png`: 마음쓰기 봉투 테마별 대표 이미지
...
마음쓰기 권한은 이메일 인증이 아니라 **strict 010 휴대전화 인증**으로만 부여합니다. 신규 카카오 가입자는 첫 가입 직후 `/phone-verification?next=/onboarding`으로 이동해 전화번호 인증을 먼저 완료하고, 기존 미인증 사용자는 기존처럼 `/write` 진입/작성 시점에만 차단됩니다. `/write`는 연락처 선택 UI를 보여주지 않고, `/api/me/contacts`의 `writerEligibility.hasVerifiedStrictPhone`만 확인합니다.

- `/phone-verification`은 전화번호 입력, 인증번호 발송/재발송, 6자리 OTP 검증, 완료 후 안전한 `next` 이동을 담당합니다.
- `/api/me.accountSetup.requiresSignupPhoneVerification=true`인 신규 가입자는 `/api/me`, `/api/me/contacts*`, `/api/auth/logout` 외 주요 인증 API에서 `409 SIGNUP_PHONE_VERIFICATION_REQUIRED`로 차단됩니다.
...
수신거부는 두 종류로 분리합니다. 공개 도착 링크 화면의 이메일/SMS 알림 수신거부는 `/api/public/notification-suppressions`와 `ContactSuppression`을 사용하며, raw 이메일/전화번호를 저장하지 않고 `PUBLIC_TOKEN_PEPPER` 기반 HMAC-SHA256 hash만 저장합니다. 이메일 수신거부는 이메일만, 문자 수신거부는 문자만 막는 채널별 정책입니다.

마이페이지 송수신 거부는 `/api/me/communication-blocks`와 `CommunicationBlock`을 사용합니다. 로그인 사용자가 회원, 이메일, 전화번호를 `SEND_TO` 또는 `RECEIVE_FROM` 방향으로 등록할 수 있고, 이후 새 마음쓰기, 로그인 답장, 친구 요청/초대 claim/수락을 `409 *_COMMUNICATION_BLOCKED`로 차단합니다. 과거 마음, 답장, 친구관계는 숨기거나 삭제하지 않습니다. 연락처 대상은 raw 값 대신 hash와 masked value만 저장합니다.

Frontend 연동 포인트:

- `/my`: “송수신 거부” 섹션에서 `GET /api/me/communication-blocks`로 목록을 불러오고, 친구 선택은 기존 `/friends`의 `userId`, 직접 입력은 EMAIL/PHONE target을 사용합니다.
- 받은 마음 거부는 `direction=RECEIVE_FROM`, 보낸 마음 거부는 `direction=SEND_TO`로 저장합니다.
- `/write`, 로그인 답장, 친구 요청/초대 화면은 `MESSAGE_COMMUNICATION_BLOCKED`, `REPLY_COMMUNICATION_BLOCKED`, `FRIEND_COMMUNICATION_BLOCKED`를 “송수신 거부 설정으로 진행할 수 없어요.” 계열 안내로 표시합니다.
- `/arrival/[token]`의 이메일/SMS 수신거부 토글은 기존 public notification suppression으로 유지합니다.
...
마음쓰기 권한은 이메일 인증과 무관합니다. 신규 가입자는 카카오 OAuth 최초 생성 시 `signupPhoneVerificationRequiredAt`가 저장되고, PHONE OTP 인증 성공 시 `signupPhoneVerificationCompletedAt`가 저장된 뒤 온보딩으로 이동합니다. 서버는 메시지 생성 시 클라이언트가 보낸 `senderContactId`를 신뢰하지 않고, 직접 인증된 PHONE contact를 조회합니다.
...
로그인 사용자가 받은 마음 상세에서 작성하는 답장은 `POST /api/messages/:id/replies`를 사용하며,
`authorUserId`가 저장됩니다. 따라서 “내가 쓴 답장”은 `/api/messages/replies/sent`,
“내가 받은 답장”은 `/api/messages/replies/received`로 분리해서 조회합니다.

인앱 알림 흐름:

- `NotificationLog.channel = IN_APP`, `status = SENT`, `targetUserId = me`인 row를 사용자 알림으로 조회합니다.
- 읽지 않은 수는 `readAt IS NULL` 기준입니다.
- 받은 마음 상세 열람, 답장 읽음 처리, 알림 단건/전체 읽음 API가 `NotificationLog.readAt`을 갱신합니다.
- AppShell 종 아이콘은 `/api/notifications/summary`의 `unreadCount`를 사용합니다.

오늘의 한 줄 흐름:

- `DailyLine`에는 OpenAI가 생성한 실제 응답을 저장합니다.
- `DailyLineSelection.date = YYYY-MM-DD`는 KST 날짜 기준입니다.
- scheduler는 `DAILY_LINE_CRON`으로 매일 OpenAI를 호출해 selection을 미리 만들고, `/api/daily-line`은 selection이 없으면 즉시 생성합니다.
- OpenAI 실패 시 `DAILY_LINE_GENERATION_FAILED`로 실패하며 mock/dummy/seed 문구는 생성하지 않습니다.
- 프롬프트는 꽃 관련 시 일부를 `글귀`, `시제목`, `시인` 순서로 저장하되, 공개저작물로 판단 가능한 고전 시만 사용하도록 제한합니다.

Frontend 연동 포인트:

- AppShell profile 왼쪽 종 아이콘은 `/api/notifications/summary`의 `unreadCount`로 badge를 표시합니다.
- `/write`는 `/api/me`의 nickname을 `senderDisplayName` 기본값으로 채우고, 전송 API 응답 전까지 loading overlay를 유지합니다.
- `/write`는 local theme option 대신 `@maeari/shared`의 `messageThemeOptions`를 사용하고, 선택값 preview는 `messageThemeEnvelopeByTheme[theme].imageUrl`로 표시합니다.
- `/sent` 답장함은 `/api/messages/replies/received`를 기본으로 사용하고, 기존 `/api/messages/sent/replies`는 deprecated alias로만 둡니다.
- `/inbox` 또는 받은 마음 상세에서 작성하는 로그인 답장은 `POST /api/messages/:id/replies`를 사용합니다.
- AppShell의 오늘의 한 줄 하드코딩은 `GET /api/daily-line` 응답으로 교체합니다.
...
회원이 /tree에서 title/description/scheduledAt 설정
  ...
  -> scheduler가 scheduledAt 이후 DELIVERED 처리 또는 owner가 PATCH /message-collections/:id/close로 즉시 도착 처리
...
```txt
20260707120000_notifications_replies_daily_line
```
...
- `MessageReply.authorUserId`, `authorDeletedAt`
- `NotificationLog.readAt`
- `DailyLine`, `DailyLineSelection`
