...
| GET | `/api/messages/received` | 받은 마음 목록. 각 message에 `thumbnail` 포함 | 세션 쿠키 | `{ messages }` |
| GET | `/api/messages/archived` | 아카이브한 받은 마음 목록. 각 message에 `thumbnail` 포함 | 세션 쿠키 | `{ messages }` |
...
- 목록 thumbnail: `/api/messages/sent`, `/api/messages/received`, `/api/messages/archived`의 각 message는 `thumbnail: { url, source, attachmentId, alt }`를 포함합니다. 첫 번째 첨부 이미지가 있으면 `source="ATTACHMENT"`와 첨부 `publicUrl`을 사용하고, 첨부가 없으면 message id 기준으로 4개 기본 이미지 중 하나를 고정 선택해 `source="DEFAULT"`로 반환합니다.
